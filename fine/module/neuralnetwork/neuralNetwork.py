## Provenance
# https://github.com/prajwalkr/SnapSudoku/blob/master/scripts/train.py
## Modify
# py2 -> py3, xrange -> range.
# modify constructor.
# add getDatasetUnitSize method.
...
class NeuralNetwork(object):
    ...
    def __init__(self, path):
        ...
        if dataSet:
            self.sizes, self.biases, self.wts = dataSet
    ...
    def loadDataSet(self, path):
        with open(path, 'rb') as in_file:
            return pickle.load(in_file, encoding='latin1')

    # TODO imp
    def getDatasetUnitSize(self):
        return (28, 28)

    def recognition(self, target):
        size = self.getDatasetUnitSize()
        resetTarget = cv2.resize(target, size)
        score = self.feedforward(np.reshape(resetTarget, (size[0] * size[1], 1)))
